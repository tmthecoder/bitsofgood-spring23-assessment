...
describe("GET - /api/admin/animals", () => {
    ...
    beforeAll(async () => {
        ...
        for(let i = 1; i < 50; i++) {
            await request(app).post("/api/animal").set("Authorization", `Bearer ${token}`).send(animalArray[i]);
        }
    })
    let lastId: string | undefined;
    test("Check if api returns 1st 10 animals", async () => {
        const page = await request(app).get('/api/admin/animals').set('Authorization', `Bearer ${token}`).send()
    ...
    test("Check the next page to test pagination", async () => {
        const page = await request(app).get(`/api/admin/animals?lastId=${lastId}`).set('Authorization', `Bearer ${token}`).send()
        expect(page.statusCode).toEqual(200);
        for(let i = 10; i < 19; i++) {
    ...
    test("Check passing in a larger page size", async () => {
        const page = await request(app).get(`/api/admin/animals?lastId=${lastId}&size=20`).set('Authorization', `Bearer ${token}`).send()
        expect(page.statusCode).toEqual(200);
        for(let i = 10; i < 29; i++) {
            expect(page.body['data'][i-10]['name']).toEqual(animalArray[i]['name'])
        }
