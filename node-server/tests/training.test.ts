...
beforeAll(async () => {
    ...
    const correctAnimalResult = await request(app).post('/api/animal').set("Authorization", `Bearer ${token}`).send({
    ...
    const wrongAnimalResult = await request(app).post('/api/animal').set("Authorization", `Bearer ${otherToken}`).send({
...
describe("POST - /api/training", () => {
    ...
    test("Create with no body", async () => {
        const result = await request(app).post("/api/training").set("Authorization", `Bearer ${token}`).send({});
...
describe("GET - /api/admin/training", () => {
    ...
    beforeAll(async () => {
        ...
        for (let i = 1; i < 50; i++) {
            await request(app).post("/api/training").set("Authorization", `Bearer ${token}`).send(trainingArray[i]);
        }
    })
    let lastId: string | undefined;
    test("Check if api returns 1st 10 animals", async () => {
        const page = await request(app).get('/api/admin/training').set('Authorization', `Bearer ${token}`).send()
        expect(page.statusCode).toEqual(200)
        for (let i = 0; i < 10; i++) {
            expect(page.body['data'][i]['description']).toEqual(trainingArray[i]['description'])
        }
        lastId = page.body['lastId'];
    })
    test("Check the next page to test pagination", async () => {
        const page = await request(app).get(`/api/admin/training?lastId=${lastId}`).set('Authorization', `Bearer ${token}`).send()
        expect(page.statusCode).toEqual(200);
        for (let i = 10; i < 19; i++) {
            expect(page.body['data'][i - 10]['description']).toEqual(trainingArray[i]['description'])
        }
    })
    test("Check passing in a larger page size", async () => {
        const page = await request(app).get(`/api/admin/training?lastId=${lastId}&size=20`).set('Authorization', `Bearer ${token}`).send()
        expect(page.statusCode).toEqual(200);
        for (let i = 10; i < 29; i++) {
            expect(page.body['data'][i - 10]['description']).toEqual(trainingArray[i]['description'])
        }
