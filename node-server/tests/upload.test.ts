...
describe("POST - /api/file/upload", () => {
  ...
  beforeAll(async () => {
    ...
    const animalResult = await request(app)
      .post("/api/animal")
    ...
    const trainingResult = await request(app)
      .post("/api/training")
      .set("Authorization", `Bearer ${token}`)
      .send({
        date: "2023-01-10",
    ...
    const otherUserAnimalResult = await request(app)
      .post("/api/animal")
    ...
    const otherUserTrainingResult = await request(app)
      .post("/api/training")
      .set("Authorization", `Bearer ${secondToken}`)
      .send({
        date: "2023-01-10",
  ...
  test("Upload profile picture", async () => {
    const result = await request(app)
      ...
      .set("Authorization", `Bearer ${token}`)
      .attach("file", "./tests/assets/user.jpg")
      .field("type", "user");
    expect(result.statusCode).toEqual(200);
    const imgCheck = await fetch(
      `${process.env.STORAGE_BUCKET_URL}/user/${userId}`
    );
  ...
  test("Upload animal picture", async () => {
    const result = await request(app)
      .post("/api/file/upload")
      .set("Authorization", `Bearer ${token}`)
      .attach("file", "./tests/assets/animal.webp")
      .field("type", "animal")
      .field("id", animalId!);
    expect(result.statusCode).toEqual(200);
    const imgCheck = await fetch(
      `${process.env.STORAGE_BUCKET_URL}/animal/${animalId}`
    );
    expect(imgCheck.status).toEqual(200); // Image is on Cloudflare R2
  });
  ...
  test("Upload training log", async () => {
    const result = await request(app)
      .post("/api/file/upload")
      .set("Authorization", `Bearer ${token}`)
      .attach("file", "./tests/assets/trainingLog.mp4")
      .field("type", "training")
      .field("id", trainingLogId!);
    expect(result.statusCode).toEqual(200);
    const imgCheck = await fetch(
      `${process.env.STORAGE_BUCKET_URL}/training/${trainingLogId}`
    );
    expect(imgCheck.status).toEqual(200); // Image is on Cloudflare R2
  });
  ...
  test("Upload training log with non-associated user token", async () => {
    const result = await request(app)
      ...
      .attach("file", "./tests/assets/trainingLog.mp4")
      .field("type", "training")
      .field("id", otherUserTrainingLogId!);
    expect(result.statusCode).toEqual(400);
  });
});
